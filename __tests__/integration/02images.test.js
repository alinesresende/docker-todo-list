...
describe(requirements[8].description, () => {
  ...
  it("A imagem 'todobackend' deve passar pelos testes de validação", async () => {
    await expect("todobackend").toPassStructureTest("test-todobackend.yml")
  })
});
...
describe(requirements[9].description, () => {
  ...
  it("A imagem 'todofrontend' deve passar pelos testes de validação", async () => {
    await expect("todofrontend").toPassStructureTest("test-todofrontend.yml")
  })
});
...
describe(requirements[10].description, () => {
  ...
  it("A imagem deve conter as camadas da imagem betrybe/puppeteer:1.0", async () => {

    const { stdout: matchingImagesJSON } = await readCommand(false, "docker image inspect todotests", false);
    const [todoTestsImageData] = JSON.parse(matchingImagesJSON)

    expect(todoTestsImageData.RootFS.Layers).toContain("sha256:39982b2a789afc156fff00c707d0ff1c6ab4af8f1666a8df4787714059ce24e7")
  })
  it("A imagem 'todotests' deve passar pelos testes de validação", async () => {
    await expect("todotests").toPassStructureTest("test-todotests.yml")
  })
});
